...
- **Contribution status values:** `active` → `accepted` or `released`, plus `release_failed`. There is no CHECK constraint and no enum on `contributions.status` — no migration has ever added one, and the only CHECK the table ever carried was `contributions_amount_range`, dropped by `20260830010000`. So an unrecognised value is accepted by the database and then silently excluded by every reader, because all nine contribution-status reads in this repo are positive filters naming the values they want. Nothing throws. A wrong value simply vanishes from the leaderboard, the display route, the contributed badge, and the performer's earnings figures, and because every read is untyped `any`, TypeScript flags none of it.
- **`release_failed` means a hold the concert close asked Stripe to cancel and could not.** The fan was NOT charged, exactly as with `released`. It counts toward the performer's Released total in `profile/page.tsx` — added 2026-09-02 in 8af0154 — and must be EXCLUDED from every `status = 'active'` read, because it is not a live request. Three Edge Functions in the iOS repo write it: `cancel-payments`, `end-concert`, `end-taking-requests`, one site each, measured 2026-09-02. A fourth, `retry-failed-releases`, reads rows at that status and retries the cancel until it succeeds or Stripe's seven-day authorization expiry drops it. Its invocation cadence lives in `cron.job` on the database, not in either repo — read it there rather than assuming.
...
- Gate every revert on a case-insensitive bare-substring grep for `zzz` across the whole file, expecting zero. Run it as `grep -in zzz FILE ; echo "exit=$?"` — **no `-F`**, because `grep -iF` aborts with exit 134 in this environment and prints nothing, and **no `|| echo` fallback**, because that turns the abort into a clean-looking pass. Exit 1 with no output is the real zero; exit 134 means the gate did not run. Both are documented in the Lessons section.
...
- **`cmd || echo "(none)"` reports "(none)" when the command DIES, not only when it finds nothing.** On 2026-09-02 a read pass over this file used that form and reported `tsc`, `baseline`, `validator.ts`, `contributions`, `status`, `released` and `lessons` as absent. All seven were present. The greps had died with exit 134 and printed nothing, so the fallback fired exactly as it would on a clean zero-match. **Never gate on a command whose failure and whose zero-result produce the same output.** Print the exit code separately — `cmd ; echo "exit=$?"` — or use a tool that cannot conflate them.
- **`grep -c` counts matching LINES, not occurrences.** Measured 2026-09-02: `grep -c status CLAUDE.md` returns 9 while `grep -o status CLAUDE.md | wc -l` returns 19, because one line carries the word several times. For an occurrence count use `grep -o PATTERN FILE | wc -l`, or count substrings on raw bytes in Python. A `-c` result must never be quoted as "N occurrences".
- **`grep -iF` ABORTS in this environment — every invocation, every file.** Measured 2026-09-02: `-c`, `-i`, `-F` and `-ni` all work alone, but any combination of `-i` and `-F` together dies with SIGABRT and exit 134, printing nothing. Reproduced on `CLAUDE.md`, `AGENTS.md`, `README.md`, `package.json`, `src/lib/supabase.ts` and `concerts/[id]/live/page.tsx`. **This, not any locale or encoding effect, is what produced the seven false negatives above** — `grep -ni tsc CLAUDE.md` finds line 40 correctly. Never combine `-i` and `-F`: use one or the other, or Python when you need both. This breaks the `zzz` revert gate above as written; that gate must not use `-iF`.
- **Bash here runs under `LC_CTYPE=C.UTF-8`, with `LANG`, `LC_ALL` and `LC_CTYPE` all unset as variables.** Measured 2026-09-02. Multibyte characters are handled correctly, but case folding is ASCII-only: `printf 'CAFE' | grep -ic cafe` returns 1 while `printf 'CAFÉ' | grep -ic café` returns 0 — `É` does not fold to `é`. So `-i` across accented text, curly quotes or Greek silently under-matches. ASCII matching around a multibyte character is unaffected — `printf 'a—b' | grep -ic a` returns 1 — which is why byte-exact anchors on em-dash lines still work. This is a separate issue from the `-iF` abort above and did not cause it.
