...
export default function NewConcertPage() {
  ...
  const { venueSuggestions, showVenueSuggestions, venueWrapperRef, handleSelectVenue, onVenueFocus } = useVenueAutocomplete({
    venueName,
    onVenueNameChange: setVenueName,
    onDetailsSelected: ({ city: cityValue, country: countryLong, state: stateLong }) => {
      if (cityValue) setCity(cityValue);
      if (countryLong) setCountry(COUNTRIES.find((c) => c === countryLong) ?? 'Other');
      if (stateLong && countryLong === 'United States') setState(US_STATES.find((s) => s === stateLong) ?? 'Alabama');
    },
  });
